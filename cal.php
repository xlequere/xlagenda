	...
	for ($i = 1; $i <= 7; $i++)
	{
		...
		else
		{
			...
			if ($num_jour <= $nombre)
			{
				...
				$search_date="$year-$month-$num_jour";
				$stmt=$connexion->prepare("SELECT id FROM $table_agenda WHERE ((date_debut <= ? AND date_fin >= ?) OR (date_debut=?)) AND actif = 1");
				$stmt->bind_param('sss',$search_date,$search_date,$search_date);
				$stmt->execute();
				$result=$stmt->get_result();
				$stmt->close();
				if ($month == $this_month && $year == $this_year && $num_jour == $this_day)
				{
					$style_day="today";
				}
				if ($result->num_rows)
